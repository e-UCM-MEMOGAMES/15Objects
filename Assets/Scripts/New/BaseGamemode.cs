...
{
    protected LevelMngr levelManager;
    public LevelMngr LevelManager
    {
    ...
    public virtual void SendAnswer(string text)
    {
        ...
        {
            if (selectedItems[i].CorrectWords.Contains(text))
            {
        ...
        if (found)
        {
            levelManager.CorrectAnswer(item);
        }
        // Si no, la respuesta es incorrecta
        else
        {
            levelManager.IncorrectAnswer();
        }
    }
}
